...
const animations: Record<string, any> = Object.keys(keyframes).map((k) => [
    `--animate-${k}`,
...
const keyframesCss = Object.entries(keyframes).map(([name, frames]) => {
    const frameStrings = Object.entries(frames).map(([percent, props]) => {
        const propStrings = Object.entries(props).map(([prop, value]) => {
            return `  ${prop}: ${value};`;
        }).join('\n');
        return `${percent} {\n${propStrings}\n}`;
    }).join('\n');
...
const prefixed = [
    ...
    {
        values: [['faster', '0.5s'], ['fast', '0.8s'], ['slow', '2s'], ['slower', '3s']],
        cssKey: 'animation-duration',
    },
    {
...
${prefixed.map(({ values, cssKey }) => values.map((val) => `@utility animate-${Array.isArray(val) ? val[0] : val} {
...
fs.writeFileSync(path.join(currentDir, '../dist', 'index.css'), code.toString());
